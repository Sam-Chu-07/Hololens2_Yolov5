...
{
    public class YoloV5Prediction : IDisposable
    {
        ...
        public IReadOnlyList<YoloResult> GetResults(string[] categories, float scoreThres = 0.5f, float iouThres = 0.5f)
        {
        ...
        private List<YoloResult> ApplyNMS(List<float[]> postProcessedResults, string[] categories, float iouThres = 0.5f)
        {
            postProcessedResults = postProcessedResults.OrderByDescending(x => x[4]).ToList(); // sort by confidence
            List<YoloResult> resultsNms = new List<YoloResult>();
            ...
            while (f < postProcessedResults.Count)
            {
                ...
                resultsNms.Add(new YoloResult(res.Take(4).ToArray(), label, conf));
